...
import { MdEdit, MdDelete } from "react-icons/md"; // Using Material UI icons from react-icons/md
...
const HomeSliderBanners = () => {
  ...
  if (isError) {
    const apiError = error as unknown as ApiError; // Safe type assertion
    return (
      ...
      <div className="flex items-center justify-between p-6 bg-white mb-6 rounded-xl shadow-sm border border-gray-200">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage your home slider banners here.
          </p>
        </div>
        <Link href="/dashboard/home-banners/add">
          <Button>+ Add Home Banners</Button>
        </Link>
      </div>

      {/* Table Container with vertical scroll if content exceeds 500px */}
                ...
                <td className="px-6 py-4 whitespace-nowrap text-center">
                  <div className="flex items-center justify-center space-x-4">
                    <Tooltip title="Edit Banner" arrow>
                      <Link href={`/dashboard/home-banners/edit/${banner.id}`}>
